...
extension NicknameSelectView{
    ...
    func duplicationAction(){
        switch isVaildInfo(){
        case .changedEmail,.notConfirmDuplicate,.success:
            vmCheck.checkNickname(nickname: vmAuth.patchUser.nickname)
            temp = vmAuth.patchUser.nickname
        default:duplicationResult = (false,isVaildInfo().message)
        }
    }
    func nextAction(){
        switch isVaildInfo(){
        case .success: withAnimation(.linear){  vmAuth.selection = .gender }
        default:duplicationResult = (false,isVaildInfo().message)
        }
    }
}
